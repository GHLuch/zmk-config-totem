...
Текущая ветка `v0.3-branch` зафиксирована на версии ZMK `v0.3`.
Если вам нужна поддержка последней версии ZMK, переключитесь на ветку `master`.
...
The current `v0.3-branch` is pinned to ZMK version `v0.3`.
If you need support for the latest ZMK version, switch to the `master` branch.
